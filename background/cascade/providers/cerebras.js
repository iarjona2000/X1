var X1ProviderCerebras = (function() {

  var BASE_URL = 'https://api.cerebras.ai/v1';
  var DEFAULT_MODEL = 'llama-3.3-70b';

  function getKey() { return (typeof aiKeys !== 'undefined' && aiKeys.cerebrasKey) || ''; }

  function complete(systemPrompt, userMsg, options) {
    return new Promise(function(resolve, reject) {
      var key = getKey();
      if (!key) { reject(new Error('CEREBRAS_KEY_NOT_SET')); return; }
      ...
      fetch(BASE_URL + '/chat/completions', {
        ...
        if (data.error) {
          if (data.error.message && data.error.message.indexOf('rate') !== -1) {
            if (typeof X1RateLimiter !== 'undefined') X1RateLimiter.setCooldown('cerebras', 15000);
          }
          reject(new Error(data.error.message || 'CEREBRAS_ERROR'));
          return;
        }
        if (typeof X1RateLimiter !== 'undefined') X1RateLimiter.recordCall('cerebras');
        var text = data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content;
        if (text) resolve(text);
        else reject(new Error('CEREBRAS_EMPTY'));
