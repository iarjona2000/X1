  ...
  function complete(systemPrompt, userMsg, options) {
    return new Promise(function(resolve, reject) {
      ...
      fetch(BASE_URL + '/chat/completions', {
        ...
        if (data.error) {
          if (typeof X1RateLimiter !== 'undefined') X1RateLimiter.setCooldown('nvidia', 15000);
          reject(new Error(data.error.message || 'NVIDIA_ERROR'));
          return;
        }
        if (typeof X1RateLimiter !== 'undefined') X1RateLimiter.recordCall('nvidia');
        var text = data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content;
        if (text) resolve(text);
        else reject(new Error('NVIDIA_EMPTY'));
