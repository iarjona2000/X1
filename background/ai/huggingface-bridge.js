...
(function() {
  ...
  HuggingFaceBridge.prototype.speak = function(text, options) {
    ...
    // Use HuggingFace API
    if (window.X1PiperBridge) {
      return window.X1PiperBridge.speak(text, opts);
    }
  ...
  HuggingFaceBridge.prototype.listen = function(duration, options) {
    ...
    // Use HuggingFace API
    if (window.X1WhisperBridge) {
      return window.X1WhisperBridge.listen(duration || 10, opts);
    }
  ...
  HuggingFaceBridge.prototype._listenLocal = function(duration, options) {
    ...
    if (window.X1WhisperBridge) {
      return window.X1WhisperBridge.listen(duration || 10, options);
    }
    return Promise.resolve({ ok: false, error: 'Whisper not available' });
  };
  ...
  HuggingFaceBridge.prototype.transcribe = function(audioInput, options) {
    ...
    if (window.X1WhisperBridge) {
      return window.X1WhisperBridge.transcribe(audioInput, opts);
    }
    return Promise.resolve({ ok: false, error: 'Whisper not available' });
  };
  ...
  HuggingFaceBridge.prototype.healthCheck = function() {
    var hasPiper = !!window.X1PiperBridge;
    var hasWhisper = !!window.X1WhisperBridge;
  ...
  if (window.X1Integrations) {
    window.X1Integrations.register({
      ...
      healthCheck: function() { return window.X1HuggingFaceBridge.healthCheck(); },
