...
**Addressed 2026-07-07:** new module `background/automation/issue-12-actions.js` (~180 líneas) implementa las 11 acciones via **runtime wrap** sobre execAction (no añade cases al switch). 8 delegan a funciones ya existentes; 3 son nuevas (`blockDistractionDomain`/`unblockDistractionDomain` para PARTE 7 firewall, `setAgentAutonomy` para PARTE 5 negociación). Cableado con una sola línea `importScripts('automation/issue-12-actions.js')` insertada antes de 'x1-integration.js' (vía `sed -i` por la limitación del patcher para el SW de 19k-line; filesystem in-place funciona sin problemas). Estado: cada case devuelve texto `showText: true` con emoji y formato YODA (≤3 líneas, decisiones binarias). Patrón wrap es 100% revertible (borrar archivo + línea del importScripts).
