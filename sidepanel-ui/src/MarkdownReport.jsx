...
function inline(text, keyPrefix) {
  ...
  return parts.map(function (p, i) {
    if (/^\*\*[^*]+\*\*$/.test(p)) {
      return React.createElement('strong', { key: keyPrefix + i, style: { fontWeight: '600', color: '#26251E' } }, p.slice(2, -2));
    }
    if (/^`[^`]+`$/.test(p)) {
      return React.createElement('code', {
        key: keyPrefix + i,
        style: { fontFamily: MONO, fontSize: '85%', background: 'rgba(175,184,193,0.2)', padding: '0.2em 0.4em', borderRadius: '4px' },
      }, p.slice(1, -1));
    }
    return p;
  });
    ...
    blocks.push(
      React.createElement('ul', { key: 'ul' + blocks.length, style: { margin: '0 0 12px', paddingLeft: '20px' } },
        items.map(function (it, i) {
          return React.createElement('li', { key: i, style: { fontSize: '13px', color: '#26251E', lineHeight: '1.7', marginBottom: '2px' } }, inline(it, 'li' + i + '-'));
        })
      )
    );
    listBuf = [];
  ...
  lines.forEach(function (raw, idx) {
    ...
    if (h) {
      ...
      blocks.push(
        React.createElement('div', {
          key: 'h' + idx,
          style: {
            fontSize: level <= 2 ? '15px' : '13px', fontWeight: '600', color: '#26251E',
            marginTop: blocks.length ? '18px' : '0', marginBottom: '8px',
            paddingBottom: '6px', borderBottom: '1px solid #F2F1ED',
          },
        }, h[2])
      );
      return;
    ...
    if (numbered) {
      ...
      blocks.push(
        React.createElement('div', {
          key: 'h' + idx,
          style: { fontSize: '15px', fontWeight: '600', color: '#26251E', marginTop: blocks.length ? '18px' : '0', marginBottom: '8px', paddingBottom: '6px', borderBottom: '1px solid #F2F1ED' },
        }, numbered[1])
      );
      return;
    }
    ...
    flushList();
    blocks.push(
      React.createElement('p', { key: 'p' + idx, style: { fontSize: '13px', color: '#26251E', lineHeight: '1.7', margin: '0 0 10px' } }, inline(line, 'p' + idx + '-'))
    );
  });
  flushList();
