...
export function checkGoogleAuth() { return new Promise(function(resolve) { if (typeof chrome === 'undefined' || !chrome.runtime?.sendMessage) { resolve(false); return; } chrome.runtime.sendMessage({ type: 'X1_AUTH_CHECK_GOOGLE' }, function(resp) { if (chrome.runtime.lastError) { resolve(false); return; } resolve(resp?.logged ?? false); }); }); }
export function loginGoogle() { return new Promise(function(resolve) { if (typeof chrome === 'undefined' || !chrome.runtime?.sendMessage) { resolve(false); return; } chrome.runtime.sendMessage({ type: 'X1_AUTH_LOGIN_GOOGLE' }, function(resp) { if (chrome.runtime.lastError) { resolve(false); return; } resolve(resp?.ok ?? false); }); }); }
