---
tags: [agente, conector, "tema/conectores", "categoria/automatizacin-e-integracin", "nivel-1"]
tier: mcp
dominio: conectores
subdominio: Automatización e integración
...
**Qué conecta:** automatización visual self-hosted, alternativa abierta a Zapier

Servidor MCP disponible en el directorio oficial de conectores de Claude (claude.com/connectors). Mismo protocolo que `X1MCPClient` ya implementa — añadirlo a X1 es `X1MCPClient.addServer({name: 'n8n', url: '<url del servidor MCP>'})` una vez el usuario tenga (o despliegue) ese servidor corriendo. No requiere código nuevo en X1, solo configuración.
