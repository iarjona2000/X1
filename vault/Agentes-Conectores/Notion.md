---
tags: [agente, conector, "tema/conectores", "categoria/productividad", "nivel-1"]
tier: mcp
dominio: conectores
subdominio: Productividad
...
**Qué conecta:** notas/wiki/bases de datos — altísimo valor, el más pedido de esta categoría

Servidor MCP disponible en el directorio oficial de conectores de Claude (claude.com/connectors). Mismo protocolo que `X1MCPClient` ya implementa — añadirlo a X1 es `X1MCPClient.addServer({name: 'Notion', url: '<url del servidor MCP>'})` una vez el usuario tenga (o despliegue) ese servidor corriendo. No requiere código nuevo en X1, solo configuración.
