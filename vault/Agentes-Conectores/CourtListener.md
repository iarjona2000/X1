---
tags: [agente, conector, "tema/conectores", "categoria/legal", "nivel-1"]
tier: mcp
dominio: conectores
subdominio: Legal
...
**Qué conecta:** jurisprudencia real, base de datos legal pública — encaja con el hueco de "investigación jurídica" del Lote 1

Servidor MCP disponible en el directorio oficial de conectores de Claude (claude.com/connectors). Mismo protocolo que `X1MCPClient` ya implementa — añadirlo a X1 es `X1MCPClient.addServer({name: 'CourtListener', url: '<url del servidor MCP>'})` una vez el usuario tenga (o despliegue) ese servidor corriendo. No requiere código nuevo en X1, solo configuración.
