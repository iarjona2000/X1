---
tags: [agente, conector, "tema/conectores", "categoria/cms-y-construccin-web", "nivel-1"]
tier: mcp
dominio: conectores
subdominio: CMS y construcción web
...
**Categoría:** CMS y construcción web

**Qué conecta:** diseño web visual con código real

Servidor MCP disponible en el directorio oficial de conectores de Claude (claude.com/connectors). Mismo protocolo que `X1MCPClient` ya implementa — añadirlo a X1 es `X1MCPClient.addServer({name: 'Webflow', url: '<url del servidor MCP>'})` una vez el usuario tenga (o despliegue) ese servidor corriendo. No requiere código nuevo en X1, solo configuración.
